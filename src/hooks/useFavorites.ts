...
export const useToggleFavorite = () => {
  ...
  return useMutation({
    mutationFn: async ({ businessCardId, isFavorite }: { businessCardId: string; isFavorite: boolean }) => {
      if (isFavorite) {
        const { error } = await supabase.from("favorites").delete().eq("user_id", user!.id).eq("business_card_id", businessCardId);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("favorites").insert({ user_id: user!.id, business_card_id: businessCardId });
        if (error) throw error;
      }
    },
